...
public sealed class CatalogAuthorizationTests : IClassFixture<BillFoundryWebApplicationFactory>
{
    private readonly BillFoundryWebApplicationFactory _factory;

    public CatalogAuthorizationTests(BillFoundryWebApplicationFactory factory)
    {
        _factory = factory;
    }

    [Fact]
    public async Task Catalog_page_redirects_unauthenticated_users_to_login()
    {
        ...
        using HttpResponseMessage response = await client.GetAsync(new Uri("/Catalog", UriKind.Relative));
...
public sealed class CatalogPageTests
{
    ...
    public CatalogPageTests(SqlServerFixture sql)
    {
        _sql = sql;
    }

    [Fact]
    public async Task Authenticated_user_can_open_the_catalog_list()
    {
        ...
        Assert.Contains("<h1>Catalog</h1>", html, StringComparison.Ordinal);
        Assert.Contains("New item", html, StringComparison.Ordinal);
    }
}
